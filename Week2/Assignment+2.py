...
def answer_three():
       df_countries= df[(df['Gold'] > 0) & (df['Gold.1']>0)]
       df_max_diff = (abs(df_countries['Gold']-df_countries['Gold.1'])/(df_countries['Gold']+df_countries['Gold.1']+df_countries['Gold.2']))
...
def answer_six():
    ...
    x = census_df[census_df['SUMLEV'] == 50].groupby('STNAME')['CENSUS2010POP'].apply(lambda x: x.nlargest(3).sum()).nlargest(3).index.values.tolist()
